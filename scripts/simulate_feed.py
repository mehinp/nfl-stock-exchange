...
def load_holdout():
    ...
    pbp = pbp.dropna(subset=['down','ydstogo','yardline_100','qtr','game_seconds_remaining','score_differential','wp'])
    return pbp
    ...
    for i in range(1, n):
        ...
        if not one_active:
            if trailing[i-1] < 0.35:
                if rise: rising_streak += 1
                else: rising_streak = 0
                ...
                if trailing[i-1] <= 0.30 and delta >= 0.15:
                    one_active = True
        if one_active:
            if drop: falling_streak += 1
            else: falling_streak = 0
